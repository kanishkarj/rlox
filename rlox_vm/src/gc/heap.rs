...
use super::{
    mark_roots,
    root::{Blob, CustomClone, MemoryBlob, Root, Trace, UniqueRoot},
};
pub struct Heap {
...
impl Heap {
    ...
    pub fn get_unique_root<T: 'static + Trace + CustomClone + Debug>(
        &self,
        val: T,
    ) -> UniqueRoot<T> {
        UniqueRoot {
    ...
    pub fn clone_unique_root<T: 'static + Trace + CustomClone + Debug>(
        &self,
        val: &UniqueRoot<T>,
    ) -> UniqueRoot<T> {
        UniqueRoot {
    ...
    pub fn collect_free<S: SystemCalls>(&self, vm: &mut VM<S>) {
        ...
        self.mem.borrow_mut().retain(|val| val.get_is_marked());
